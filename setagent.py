...
class SetAgent(SimpleAgent):
    ...
    def pairwise(self, a, b):
        to_step, to_flag = set(), set()
        av, bv = self.game.cell_value(a), self.game.cell_value(b) # get cell values
        an, bn = self.game.neighbours(a), self.game.neighbours(b) # get neighbours
        af, bf = an & self.game.flagged, bn & self.game.flagged # get (flagged) neighbours
        vdiff = av - bv # difference in cell value
        fdiff = len(af) - len(bf) # difference in flag count
        amb = an - bn # neighbours of A excluding neighbours of B
        bma = bn - an # neighbours of B excluding neighbours of A
    ...
    def play(self, **kwargs):
        ...
        while len(self.game.grid - self.game.flagged - self.game.stepped) > 0 and running:
            ...
            while len(to_step) > 0:
                ...
                state_changed = True
            if verbosity > 2:
                self.game.draw(show_mines=show_mines, highlighted=list(to_pair_search) if show_strategy else None, underlined=list(to_search) if show_strategy else None)
                if step_by_step:
                    input()
        ...
        if verbosity > 1:
            self.game.draw(show_mines=show_mines)
        if verbosity > 0:
